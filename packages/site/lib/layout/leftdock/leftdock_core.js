...
let coreComputeLayout;

function computeLayout(arg0) {
  var {container: v0_0, panes: v0_1, layout: v0_2 } = arg0;
  var {x: v1_0, y: v1_1, w: v1_2, h: v1_3 } = v0_0;
  var vec7 = v0_1;
  var len7 = vec7.length;
  var result7 = realloc1(0, 0, 4, len7 * 32);
  for (let i = 0; i < vec7.length; i++) {
    const e = vec7[i];
    const base = result7 + i * 32;var {id: v2_0, weight: v2_1, min: v2_2, max: v2_3 } = e;
  ...
  var ptr8 = utf8Encode(v0_2, realloc1, memory0);
  var len8 = utf8EncodedLen;
  _debugLog('[iface="leftdock:layout/core", function="compute-layout"] [Instruction::CallWasm] (async? false, @ enter)');
  const _wasm_call_currentTaskID = startCurrentTask(0, false, 'coreComputeLayout');
  const ret = coreComputeLayout(toInt32(v1_0), toInt32(v1_1), toUint32(v1_2), toUint32(v1_3), result7, len7, ptr8, len8);
  endCurrentTask(0);
  var len10 = dataView(memory0).getUint32(ret + 4, true);
  var base10 = dataView(memory0).getUint32(ret + 0, true);
  var result10 = [];
  for (let i = 0; i < len10; i++) {
    const base = base10 + i * 24;
    ...
    result10.push({
      id: result9,
      x: dataView(memory0).getInt32(base + 8, true),
      y: dataView(memory0).getInt32(base + 12, true),
      w: dataView(memory0).getInt32(base + 16, true) >>> 0,
      h: dataView(memory0).getInt32(base + 20, true) >>> 0,
    });
  }
  _debugLog('[iface="leftdock:layout/core", function="compute-layout"][Instruction::Return]', {
    funcName: 'compute-layout',
    paramCount: 1,
    postReturn: true
  });
  const retCopy = result10;
...
const $init = (() => {
  let gen = (function* _initGenerator () {
    const module0 = fetchCompile(new URL('./leftdock_core.core.wasm', import.meta.url));
    const module1 = fetchCompile(new URL('./leftdock_core.core2.wasm', import.meta.url));
    ...
    postReturn0 = exports1['cabi_post_leftdock:layout/core#compute-layout'];
    coreComputeLayout = exports1['leftdock:layout/core#compute-layout'];
  })();
...
export { core, core as 'leftdock:layout/core',  }