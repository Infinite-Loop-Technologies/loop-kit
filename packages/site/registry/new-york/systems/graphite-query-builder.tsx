...
export function GraphiteQueryBuilder({
    ...
    const addRule = () => {
        ...
        const operators =
            fallbackField.operators ?? DEFAULT_OPERATORS[fallbackField.type];
    ...
    const updateRule = (
        ruleId: string,
        updater: (rule: QueryBuilderRule) => QueryBuilderRule,
    ) => {
        onChange({
            ...value,
            ...
            {value.rules.length === 0 ? (
                ...
                <div className='space-y-2'>
                    {value.rules.map((rule) => {
                        ...
                        const operators =
                            field.operators ?? DEFAULT_OPERATORS[field.type];
                        const hidesValue =
                        ...
                        return (
                            <div
                                ...
                                <select
                                    ...
                                    onChange={(event) => {
                                        const nextField =
                                            fieldMap.get(event.target.value) ??
                                            fallbackField;
                                        if (!nextField) return;
                                        const nextOperators =
                                            nextField.operators ??
                                            DEFAULT_OPERATORS[nextField.type];
                                        const nextOperator =
                                ...
                                <Input
                                    ...
                                    disabled={hidesValue}
                                    placeholder={
                                        hidesValue ? 'No value needed' : 'Value'
                                    }
                                />
