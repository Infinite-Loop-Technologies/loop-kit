import { GraphiteRuntime, asNodeId, asScopeId, } from '../index.js';
function benchCommitThroughput(runtime, iterations) {
...
function benchQueryRerunCost(runtime, iterations) {
    ...
    scope.queryEngine.subscribe((query) => query.getProp(nodeId, 'value'), () => { });
    const start = performance.now();
    for (let index = 0; index < iterations; index += 1) {
        runtime.commitIntentPatch({
...
function benchInvalidationSelectivity(runtime) {
...
function benchValidatorCacheRatio(runtime) {
