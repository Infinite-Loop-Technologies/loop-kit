import { $set, type GraphPath, type GraphState, type GraphiteStore, type MutationPatchObject } from '@loop-kit/graphite';
import {
  applyDockPolicy,
  migrateDockState,
  reduceDockIntent,
...
function isMovePayload(value: unknown): value is DockMovePanelIntent {
  ...
  if (typeof value.panelId !== 'string' || value.panelId.trim().length === 0) return false;
  const target = value.target;
  if (!isRecord(target)) return false;
  if (typeof target.groupId !== 'string' || target.groupId.trim().length === 0) return false;
  if (
    target.zone !== 'center' &&
    target.zone !== 'tabbar' &&
    target.zone !== 'left' &&
...
function setPatchAtPath(path: GraphPath, value: unknown): MutationPatchObject | ReturnType<typeof $set> {
  if (path.length === 0) {
    return $set(value);
  }
...
export function registerDockIntents<TState extends GraphState = GraphState>(
  ...
  const unsubs = [
    store.registerIntent(intentNames.activatePanel, (payload: DockActivatePanelIntent, ctx) => {
    ...
    store.registerIntent(intentNames.movePanel, (payload: DockMovePanelIntent, ctx) => {
    ...
    store.registerIntent(intentNames.resize, (payload: DockResizeSplitIntent, ctx) => {
    ...
    store.registerIntent(intentNames.removePanel, (payload: DockRemovePanelIntent, ctx) => {
    ...
    store.registerIntent(intentNames.addPanel, (payload: DockAddPanelIntent, ctx) => {
      const dockState = resolveDockState(ctx.state, path, options.selectDock);
