...
import type { DockPreviewIntent, DockStateSlice } from '../../facet/queries.js';
...
export function createDragTabRecognizer(): Recognizer {
    return {
        id: 'dock.dragTab',
        onInput: (input, ctx) => {
            ...
            const session = ctx.getSessionState<DragTabSession>(SESSION_KEY);
            if (!session) {
                return;
            }
            ...
            if (input.kind === 'pointermove') {
                ...
                if (preview.zoneId !== session.lastZoneId) {
                    const dockState = ctx.getStateView().getSlice<DockStateSlice>(DOCK_FACET);
                    const rootId = dockState?.rootIds[0];
                    if (rootId) {
                        ctx.clearOverlay();
                        ctx.pushOverlayPatch(buildPreviewPatch(rootId, preview));
                        ctx.setSessionState<DragTabSession>(SESSION_KEY, {
                            ...session,
            ...
            if (input.kind === 'pointerup') {
                const state = ctx.getStateView().getSlice<DockStateSlice>(DOCK_FACET);
...
function toPreviewIntent(
    hit: HitTestResult | undefined,
    session: DragTabSession,
...
function toDockRegion(value: unknown, regionType: string): DockDropRegion | undefined {
...
function toNodeId(value: unknown): NodeId | undefined {
