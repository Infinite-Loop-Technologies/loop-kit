import { asNodeId, type IntentSnapshotReader, type NodeId, type Patch } from '@loop-kit/graphite-core';
...
export function buildPreviewPatch(rootId: NodeId, preview: DockPreviewIntent): Patch {
    return {
        ops: [
            {
                kind: 'setProp',
...
export function buildClearPreviewPatch(rootId: NodeId): Patch {
    return {
        ops: [
            {
                kind: 'delProp',
...
export function buildMoveTabPatch(
    snapshot: IntentSnapshotReader,
    args: MoveTabPatchArgs,
): Patch {
    const ops: Patch['ops'] = [];
...
export function buildSplitTabPatch(
    snapshot: IntentSnapshotReader,
    args: SplitTabPatchArgs,
): Patch {
...
export function buildPatchFromPreview(
    snapshot: IntentSnapshotReader,
    preview: DockPreviewIntent,
    options: { newGroupId?: NodeId } = {},
): Patch {
