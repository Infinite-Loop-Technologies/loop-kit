...
import {
    ...
    DOCK_PROP_PREVIEW,
    DOCK_PROP_SPLIT_DIRECTION,
    DOCK_PROP_SPLIT_RATIOS,
...
export function normalizeDockSnapshot(snapshot: IntentSnapshotReader): DockNormalizationResult {
    const diagnostics: Diagnostic[] = [];
    const rootIds = [...snapshot.nodesByType(DOCK_NODE_ROOT)];
    const groupIds = [...snapshot.nodesByType(DOCK_NODE_GROUP)];
    const groups: Record<string, DockNormalizedGroup> = {};
    ...
    for (const groupId of groupIds) {
        ...
        const splitDirection = normalizeSplitDirection(
            groupId,
            snapshot.getProp(groupId, DOCK_PROP_SPLIT_DIRECTION),
            diagnostics,
        );
        const splitRatios = normalizeSplitRatios(
            snapshot.getProp(groupId, DOCK_PROP_SPLIT_RATIOS),
            childGroupIds.length + 1,
            groupId,
            diagnostics,
        );
    ...
    const rootGroupIds = rootIds.flatMap((rootId) =>
        snapshot
    ...
    if (rootIds.length === 0) {
        diagnostics.push(
            dockWarning(
                DOCK_DIAG_CODES.ROOT_MISSING,
                'No dock.root node exists. Layout will render from orphan groups.',
            ),
        );
    }

    const layoutIR = buildLayoutIR(
        rootGroupIds.length > 0 ? rootGroupIds : groupIds,
        groups,
        diagnostics,
    );
...
function normalizeActiveTab(
    ...
    diagnostics.push(
        dockWarning(
            DOCK_DIAG_CODES.INVALID_ACTIVE_TAB,
            'Group active tab is invalid. Falling back to first tab.',
            groupId,
        ),
    );
    return tabIds[0];
...
function normalizeSplitDirection(
    ...
    diagnostics.push(
        dockWarning(
            DOCK_DIAG_CODES.INVALID_SPLIT,
            'Group split direction is invalid. Ignoring split direction.',
            groupId,
        ),
    );
    return undefined;
...
function normalizeSplitRatios(
    ...
    const ratios = value.filter((item): item is number => typeof item === 'number' && item > 0);
    if (ratios.length !== expectedLength) {
        diagnostics.push(
            dockWarning(
                DOCK_DIAG_CODES.INVALID_SPLIT,
                'Split ratios are invalid. Falling back to equal ratios.',
                groupId,
            ),
        );
        return equalRatios(expectedLength);
...
function equalRatios(length: number): number[] {
    const ratio = 1 / length;
    return Array.from({ length }, () => ratio);
}

function buildLayoutIR(
    roots: readonly NodeId[],
    groups: Record<string, DockNormalizedGroup>,
    diagnostics: Diagnostic[],
): DockLayoutNode[] {
    const forest: DockLayoutNode[] = [];
    const visited = new Set<NodeId>();

    const visit = (groupId: NodeId, path: Set<NodeId>): DockLayoutNode | undefined => {
        ...
        if (path.has(groupId)) {
            diagnostics.push(
                dockWarning(
                    DOCK_DIAG_CODES.GROUP_CYCLE,
                    'Group containment cycle detected. Child traversal stopped at cycle edge.',
                    groupId,
                ),
            );
            return undefined;
