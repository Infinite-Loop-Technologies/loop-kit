...
/**
 * Defines a literal kind helper for runtime-readable discriminants.
 *
 * Useful for target kinds and other small vocabularies where the literal itself
 * is the important contract.
 */
export const defineKind = <Kind extends TypeName>(kind: Kind): LiteralType<Kind> =>
  literal(kind);

/**
 * Defines a typed discriminated object for kinded payloads.
 *
 * This is a thin readability helper over {@link typed}, intended for runtime
 * target payloads where callers want the more domain-specific "kind" naming.
 */
export const defineTypedKind = <
  Kind extends TypeName,
  Props extends Record<string, AnyType>,
>(
  kind: Kind,
  props: Props,
): TypedType<Kind, Props> => typed(kind, props);
